...
/*
   if (m_x == vertices[0] || m_x == vertices[4]) {
     if (m_y == vertices[1] && m_y == vertices[3]) {
       printf("Scale\n");
       g_bTranslate = false;
       g_bRotate = false;
       g_bScale = true;
       }
   } else if (m_x < vertices[0] && m_x > vertices[4]) {
        if (m_y < vertices[1] && m_y > vertices[3]) {
       printf("Translate\n");
          float rate = 0.01;
          float xlen = m_x - g_pos_x;
          float ylen = m_y - g_pos_y;
          float distance = sqrt(pow(xlen,2) + pow(ylen, 2));
          while (distance > 0.01) {
            //printf("%f\n", distance);
               g_pos_x = g_pos_x + (xlen*rate);
               g_pos_y = g_pos_y + (ylen*rate);
               updateVertices();
               xlen = m_x - g_pos_x;
               ylen = m_y - g_pos_y;
               distance = sqrt(pow(xlen,2) + pow(ylen, 2));
               printf("%f, %f, %f, %f, %f\n", g_pos_x, g_pos_y, m_x, m_y, distance);
       ...
       if (x < 1 and x > -1 and y < 1 and y > -1){
         if (x > 0.9 or x < -0.9 or y > 0.9 or x < 0.9){
           printf("SCALE\n");
           g_bScale = true;
         }
         printf("TRANSLATE\n");
